function [] = plot_PID_with_PF(savee)

    % Plotting the simulation for the Design Project
    % PID controller with PreFilter
    % EE-386 - Spring 2019
    % Hunter Phillips

    addpath('../src/simulations')
    sim('PID_with_PreFilter_out.slx')
    ...
    if savee == true
        print(f1,'../results/pid_with_pf_command_pos', '-dpng', '-r1200')
        print(f2,'../results/pid_with_pf_arma_v', '-dpng', '-r1200')
        print(f3,'../results/pid_with_pf_actual_pos', '-dpng', '-r1200')
        print(f4,'../results/pid_with_pf_angle_error', '-dpng', '-r1200')
        print(f5,'../results/pid_with_pf_disturb', '-dpng', '-r1200')
    end
